...
def signup():
    if request.method == 'POST':
        ...
        flash('Successfully signed up, Sign in to continue', 'success')
        return redirect(url_for('auth.login'))
...
def forgot_password():
    if request.method == 'POST':
        ...
        flash(
            'An email has been sent to this address with instruction to reset your password.',
            'info',
        )
        return redirect(url_for('auth.login'))
